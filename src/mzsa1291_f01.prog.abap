...
FORM get_meal_info USING VALUE(p_carrid)
  ...
  DATA ls_vendor_info TYPE zssa0093.
...
FORM get_vendor_info  USING    VALUE(p_flag)
                               VALUE(p_code1)
                               VALUE(p_code2)
                      CHANGING ps_info TYPE zssa0093.
  DATA: BEGIN OF ls_cond,
          lifnr  TYPE ztsa0099-lifnr,
          carrid TYPE ztsa0099-carrid,
          mealno TYPE ztsa0099-mealnumber,
        END OF ls_cond.
  CASE p_flag.
    WHEN 'V'. "Vendor
